  ...
  if args.save_data:
    if args.nb_real_byz > 0:
      if args.gar in {'SM', 'TSM'}:
        args.result_directory = 'results_cifar/%s_attack/%s_%dagents_%dfreal_%dfdecl_T%d' % (
        args.attack, args.gar, args.nb_workers, args.nb_real_byz, args.nb_decl_byz, args.temperature)
      else:
        args.result_directory = 'results_cifar/%s_attack/%s_%dagents_%dfreal_%dfdecl' % (
        args.attack, args.gar, args.nb_workers, args.nb_real_byz, args.nb_decl_byz)
    else:
      if args.gar in {'SM', 'TSM'}:
        args.result_directory = 'results_cifar/noAttack/%s_%dagents_%dfreal_%dfdecl_T%d' % (
        args.gar, args.nb_workers, args.nb_real_byz, args.nb_decl_byz, args.temperature)
      else:
        args.result_directory = 'results_cifar/noAttack/%s_%dagents_%dfreal_%dfdecl' % (
        args.gar, args.nb_workers, args.nb_real_byz, args.nb_decl_byz)
  else:
...
with tools.Context("training", "info"):
  ...
  while not exit_is_requested():
    ...
    if args.gar == 'CC':
      ...
      grad_defense = defense.checked(gradients=(grad_honests_gar + grad_attacks), f=args.nb_decl_byz, model=model, T=args.temperature, **args.gar_args)
...
if args.gar in ['TSM', 'SM']:
  ...
  string = ('\n' + args.dataset + ' ' + args.gar + ' ' + 'n=' + str(args.nb_workers) + ' ' + 'fdecl=' + str(args.nb_decl_byz) +
            ' ' + 'freal=' + str(args.nb_real_byz) + ' ' + args.attack + ' ' + str(cosine_angle.item()))
print('string', string)
file_angle = open('results_alpha_cifar', "a")
