...
__all__ = ["full", "conv", "logit", "linear", "full_har"]
...
# simple fully connected model, for HAR
class _Full_har(torch.nn.Module):
  """ Simple, small fully connected model.
  """
  ...
  def __init__(self):
    ...
    super().__init__()
    # Build parameters
    self._f1 = torch.nn.Linear(561, 50)
    self._f2 = torch.nn.Linear(50, 6)
  ...
  def forward(self, x):
    ...
    """
    # Forward pass
    x = torch.nn.functional.relu(self._f1(x.view(-1, 561)))
    x = torch.nn.functional.log_softmax(self._f2(x), dim=1)
    return x

def full_har(*args, **kwargs):
  """ Build a new simple, fully connected model.
  Args:
    ... Forwarded (keyword-)arguments
  Returns:
