...
def process_commandline():
  ...
  # # Create subparsers for the --gar argument
  # subparsers_gar = parser.add_subparsers(dest='gar_subcommand', help='Subcommands for --gar')
  # # Subparser for the first subcommand under --gar
  # gar_m = subparsers_gar.add_parser('arg1', help='Description of subcommand1 under --gar')
  # gar_m.add_argument("--m", type=int, help="Argument for number of grads to include (m)")
  # # Subparser for the second subcommand under --gar
  # gar_T = subparsers_gar.add_parser('arg2', help='Description of subcommand2 under --gar')
  # gar_T.add_argument("--T", type=str, help="Argument for temperature for --gar")
...
with tools.Context("cmdline", "info"):
  ...
  # print(args.arg1)
  if args.save_data:
    if args.nb_real_byz > 0:
      if args.gar in {'SM', 'TSM'}:
        args.result_directory = 'results_mnist/%s_attack/%s_%dagents_%dfreal_%dfdecl_T%d_%smomentum' % (args.attack, args.gar, args.nb_workers, args.nb_real_byz, args.nb_decl_byz,
                                                                                             args.temperature, args.momentum_at)
      else:
        args.result_directory = 'results_mnist/%s_attack/%s_%dagents_%dfreal_%dfdecl_%smomentum' % (args.attack, args.gar, args.nb_workers, args.nb_real_byz,
                                                                                                    args.nb_decl_byz, args.momentum_at)
    else:
      if args.gar in {'SM', 'TSM'}:
        args.result_directory = 'results_mnist/noAttack/%s_%dagents_%dfreal_%dfdecl_T%d_%smomentum' % (args.gar, args.nb_workers, args.nb_real_byz, args.nb_decl_byz,
                                                                                                       args.temperature, args.momentum_at)
      else:
        args.result_directory = 'results_mnist/noAttack/%s_%dagents_%dfreal_%dfdecl_%smomentum' % (args.gar, args.nb_workers, args.nb_real_byz, args.nb_decl_byz, args.momentum_at)
  else:
    ...
    try:
      ...
      try:
        ...
        if args.evaluation_delta > 0:
          result_make("eval", "Step number", "Cross-accuracy", "Training-loss")
        # Make study file
        if args.nb_for_study > 0:
...
with tools.Context("training", "info"):
  ...
  while not exit_is_requested():
    ...
    if milestone_evaluation:
      ...
      try:
        loss = sum(loss_sampleds) / len(loss_sampleds)
      except:
        loss = math.nan
      print(f" {loss:.2f}.")
      # # compute angle alpha
      # print(f"Angle of true cone (step {steps})...", end="", flush=True)
      # try:
      #   aggregated_gradient_norm = grad_defense.norm().item()
      #   true_gradient = torch.mean(torch.stack(grad_honests), 0)
      #   true_gradient_norm = true_gradient.norm().item()
      #   cosine_angle = ((torch.dot(grad_defense, true_gradient)) / (true_gradient_norm * aggregated_gradient_norm))
      # except:
      #   cosine_angle = math.nan
      # print(f" {cosine_angle:.2f}.")
      # Store the evaluation result
      if fd_eval is not None:
        result_store(fd_eval, steps, acc, loss)
    ...
    if args.nb_local_steps == 1: # Standard SGD (fast path: avoid taking a deep snapshot)
      ...
      if args.momentum_nesterov:
        ...
        for _ in range(args.nb_honests):
          grad, loss = model.backprop(outloss=True)
          # Loss append
          loss_sampleds.append(loss.clone().detach_().item())
          # Gradient clip and append
          if args.gradient_clip is not None:
            ...
            if grad_norm > args.gradient_clip:
              grad.mul_(args.gradient_clip / grad_norm)
          grad_sampleds.append(grad.clone().detach_())
      for _ in range(args.nb_decl_byz):
        grad, loss = model.backprop(outloss=True)
        if args.gradient_clip is not None:
          grad_norm = grad.norm().item()
          if grad_norm > args.gradient_clip:
            grad.mul_(args.gradient_clip / grad_norm)
        grad_others.append(grad.clone().detach_())
    ...
    # print('honest grads', len(grad_sampleds))
    # print('attack', type(grad_others), 'leng', len(grad_others))
    ...
    # print('before attack')
    grad_attacks = attack.checked(grad_honests=grad_honests_gar, grad_others=grad_others, f_decl=args.nb_decl_byz, f_real=args.nb_real_byz, model=model, defense=defense, **args.attack_args)
    # ------------------------------------------------------------------------ #
    ...
    grad_defense = defense.checked(gradients=(grad_honests_gar + grad_attacks), momentum = grad_defense_old, f=args.nb_decl_byz, model=model, **args.gar_args)
    grad_defense_old = grad_defense
    # print('grad defense', grad_defense.shape)     # aggregated gradient
    ...
    #   # honest_grad_avg is the average of honest gradients
    ...
    #   # angle between aggregated vector and the true gradient
...
true_gradient_norm = true_gradient.norm().item()
cosine_angle = (torch.dot(grad_defense, true_gradient)) / (true_gradient_norm * aggregated_gradient_norm)
print('cos', cosine_angle.item())
if args.gar in ['TSM', 'SM']:
  string = ('\n' + args.dataset + ' ' + args.gar + str(args.temperature) + ' ' + 'n=' + str(args.nb_workers) + ' ' + 'fdecl=' + str(args.nb_decl_byz) +
            ' ' + 'freal=' + str(args.nb_real_byz) + ' ' + args.attack + ' ' + str(cosine_angle.item()))
else:
  string = ('\n' + args.dataset + ' ' + args.gar + ' ' + 'n=' + str(args.nb_workers) + ' ' + 'fdecl=' + str(args.nb_decl_byz) +
            ' ' + 'freal=' + str(args.nb_real_byz) + ' ' + args.attack + ' ' + str(cosine_angle.item()))
print('string', string)
file_angle = open('results_alpha_mnist', "a")
file_angle.write(string)
file_angle.close()


# print('loss_sampleds', len(loss_sampleds))
# training_loss = math.nan if loss_sampleds is None else sum(loss_sampleds) / len(loss_sampleds)
# print(training_loss)
# string_loss = '\n' + args.dataset + ' ' + args.gar + ' ' + str(args.nb_workers) + ' ' + str(args.nb_real_byz) + ' ' + str(training_loss)
# print('string', string_loss)
# file_angle = open('training_loss', "a")
# file_angle.write(string_loss)
# file_angle.close()