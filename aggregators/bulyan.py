...
def aggregate(gradients, f, m=None, **kwargs):
  ...
  selected1 = torch.empty(n - 2 * f - 2, d, dtype=gradients[0].dtype, device=gradients[0].device)
  for i in range(selected.shape[0]):
    # Update 'm'
    m = min(m, m_max - i)
    # Compute the average of the selected gradients
    scores.sort(key=lambda x: x[0])
    # print('i=', i, 'm=', m, 'scores', (scores[:m]))
    # selected[i] = sum(gradients[gid] for _, gid in scores[:m]).div_(m)
    selection_list = []
    for _, id in scores[:m]:
      gid = 0 if id is None else id
      selection_list.append(gradients[gid])
    selected1[i] = sum(selection_list).div_(m)
    # print('type', check.shape, selected[i].shape)
    # assert torch.equal(selected[i], check)
